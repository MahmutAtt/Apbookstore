...
const QAnswers =  [ //arry olusturduk
        {
 ...
 export const QuAn = () => {
  return (
    <section className='bg-center bg-no-repeat rounded-t1-1g rounded-br text-gray-900 bg-gray-50 dark:bg-gray-900 dark:text-white'>
    <div className='px-4 mx-auto max-w-screen-xl text-center py-24 lg:py-32'>
      <h1 className='mb-16 text-4xl font-extrabold tracking-tight leading-none  md:text-5xl lg:text-6xl'>{title}</h1>
              ...
              <Accordion collapseAll className='border-none'>
                  { 
                      QAnswers.map(({ question, answer}, index) => (
                          <AccordionPanel key={index}>
                              <AccordionTitle className='text-gray-900 text-xl'>{ question }</AccordionTitle>
                              <AccordionContent>
                                  <p className='mb-2 text-left text-gray-700 dark:text-yellow-400'>
                                  { answer }
                                  </p>
                              </AccordionContent>
                          </AccordionPanel>
